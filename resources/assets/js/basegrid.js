...
var getchcw=function(){
    var ch=window.screen.height -450;
    var cw=document.body.clientWidth-250;
    return {'height':ch,'width':cw}
}
...
//		$(this.dialogPannelId).modal();
        ...
        dialog.onShown(function(){
            ...
            if(typeof(self.showDone) == "function"){
                var callback = self.showDone;
                self.showDone= undefined;
                setTimeout(function(){
                     callback(dialog,sRow);
                 },500);
            }
        });
        dialog.open();
	...
	pub : function(topic, data) {
        var self=this;
		if (self.Events) {
			self.Events.pub(topic+data.action+data.event, data);
		}
	},
